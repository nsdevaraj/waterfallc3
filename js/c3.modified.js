...
c3_chart_internal_fn.initChartElements = function () {
    ...
    if (this.initWaterfallLine) { this.initWaterfallLine(); }
...
c3_chart_internal_fn.redraw = function (options, transitions) {
    ...
    var drawArea, drawBar, drawLine, xForText, yForText, drawWaterfallLine;
...
c3_chart_internal_fn.classWaterfallLine = function (d) {
	return this.classShape(d) + this.generateClass(CLASS.waterfallLine, d.index);
}; 
c3_chart_internal_fn.classWaterfallLines = function (d) {
	return this.classShapes(d) + this.generateClass(CLASS.waterfallLines, d.id);
};
c3_chart_internal_fn.classChartWaterfallLine = function (d) {
	return CLASS.chartWaterfallLine + this.classTarget(d.id);
};
    ...
    targets = ids.map(function (id, index) {
        ...
        return {
            ...
            values: data.map(function (d, i) {
				...
				if(config.data_waterfallData && config.data_waterfallData[index][0]!=='x'){
...
c3_chart_internal_fn.initWaterfallLine = function () {
	var $$ = this;
	$$.main.select('.' + CLASS.chart).append("g")
		.attr("class", CLASS.chartWaterfallLines);
}; 
c3_chart_internal_fn.updateTargetsForWaterfallLine = function (targets) {
	var $$ = this, config = $$.config,
		mainWaterfallLineUpdate, mainWaterfallLineEnter,
		classChartWaterfallLine = $$.classChartWaterfallLine.bind($$),
		classWaterfallLines = $$.classWaterfallLines.bind($$),
		classFocus = $$.classFocus.bind($$);
	mainWaterfallLineUpdate = $$.main.select('.' + CLASS.chartWaterfallLines).selectAll('.' + CLASS.waterfallLine)
		.data(targets)
		.attr('class', function (d) { return classWaterfallLines(d) + classFocus(d); });
	mainWaterfallLineEnter = mainWaterfallLineUpdate.enter().append('g')
		.attr('class', classChartWaterfallLine)
		.style('opacity', 0)
		.style("pointer-events", "none");
	// Bars for each data
	mainWaterfallLineEnter.append('g')
		.attr("class", classWaterfallLines)
		.style("cursor", function (d) { return config.data_selection_isselectable(d) ? "pointer" : null; });
};
c3_chart_internal_fn.updateWaterfallLine = function (durationForExit) {
	var $$ = this,
	...
	$$.mainWaterfallLine = $$.main.selectAll('.' + CLASS.waterfallLines).selectAll('.' + CLASS.waterfallLine)
		.data(barData);
	$$.mainWaterfallLine.enter().append('path')
		.attr("class", classWaterfallLine)
		.style("stroke", color)
		.style("fill", color);
	$$.mainWaterfallLine
		.style("opacity", initialOpacity);
	$$.mainWaterfallLine.exit().transition().duration(durationForExit)
		.style('opacity', 0)
		.remove();
};
...
c3_chart_internal_fn.redrawWaterfallLine = function (drawWaterfallLine, withTransition) {
        var $$ = this, color;
        if($$.config.data_waterfallLineColor) {
            color = $$.config.data_waterfallLineColor;
        } else {
        ...
        return [
            (withTransition ? this.mainWaterfallLine.transition(Math.random().toString()) : this.mainWaterfallLine)
                .attr('d', drawWaterfallLine) 
                .style("fill", color)
                .style("opacity", 1)
        ];
...
c3_chart_internal_fn.generateDrawWaterfallLine = function (barIndices, isSub) {
        var $$ = this, config = $$.config,
            getPoints = $$.generateGetBarPoints(barIndices, isSub);
        var holdCoords = []; 
        return function (d, i) {
